...
leaveToIsland_pre b@(BridgeOpened lim nbTo _ _) = nbTo > 0 && nbCars b < lim
leaveToIsland_pre (BridgeClosed lim nbTo nbFrom) = lim == nbTo + nbFrom 
...
enterFromIsland_pre b@(BridgeOpened lim nbTo nbI _) = nbI > 0 && nbCars b < lim
enterFromIsland_pre (BridgeClosed lim nbTo nbFrom) = lim > (nbFrom + nbTo)

leaveFromIsland :: IslandBridge -> IslandBridge
leaveFromIsland (BridgeOpened lim nbTo nbI nbFrom) = mkBridge lim nbTo nbI (nbFrom - 1)
leaveFromIsland (BridgeClosed _ _ _) = error "Cannot leave bridge from island"

leaveFromIsland_pre :: IslandBridge -> Bool
leaveFromIsland_pre b@(BridgeOpened _ _ _ nbFrom) = nbFrom > 0
