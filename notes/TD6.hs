...
-- preuve
    ...
    prop_comp f g Nil 
=   listMap (g . f) Nil == (listMap g . listMap f) Nil -- équation de prop_law_Functor_comp
=   Nil == (listMap g . listMap f) Nil -- première équation de listMap
=   Nil == listMap g (listMap f Nil)  -- équation de .
=   Nil == listMap g Nil -- première équation de listMap
=   Nil == Nil -- première équation de listMap
=   True

    prop_comp f g l
=   listMap (g . f) l == (listMap g . listMap f) l
=   listMap (g . f) l == listMap g (listMap f l)

    prop_comp f g (Cons e l)
=   listMap (g . f) (Cons e l) == listMap g (listMap f (Cons e l))
=   Cons ((g . f) e) (listMap (g . f) l) == 
=                                        == listMap g (Cons (f e) (listMap f l))
=                                        == Cons (g (f e)) (listMap g (listMap f l))
=   Cons ((g . f) e) (listMap (g . f) l) == Cons (g (f e)) (listMap g (listMap f l))
=   (listMap (g . f) l) == (listMap g (listMap f l)) -- qui est démontré au dessus
