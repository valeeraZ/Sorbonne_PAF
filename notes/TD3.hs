...
Type 
    prop_inv: Invariant - doit être vérifié pour les types, soit vrai pour tout le temps
Fonction 
    prop_pre: Précondition, doit être vérifié pat les arguments  
    prop_post: Postcondition, doit être vérifié par tous les arguments qui valident les préconditions
...
prop_SystemeInv Systeme {membres = m, projets = p} = map_pourtout prop_MembreInv m
...
prop_MembreInv Membre {prenom = p, nom = n, login = (LoginInfo l), mele = (AdresseMail a)} = not (Text.empty p) &&  not (Text.empty n) && not (Text.empty l) && not (Text.empty a)
